...
// vsockCIDAny is VMADDR_CID_ANY: bind the listener to every context ID this VM
// has or comes to have, rather than to one of them.
//
// A restored VM changes CID underneath its sockets - it carries the template's
// in its kernel command line and is handed its own by QEMU, which the guest
// adopts when the post-migration transport reset arrives - and a listener bound
// to a concrete CID is deaf from that moment on. Binding it to a concrete CID is
// what mdlayher's vsock.Listen does, and the restored guest was unreachable
// until the CID change was noticed and the listener rebuilt: 20 ms of polling to
// discover something this makes a non-event.
...
func New(ctx context.Context, cfg *config.ServiceConfig) (Runnable, error) {
	...
	// Bind to VMADDR_CID_ANY rather than to the CID the kernel command line
	// named at boot. The two agree for a VM that booted normally, and they do not
	// for a VM restored from a template: it carries the template's CID in its
	// command line while its vhost-vsock device was given one of its own, and the
	// guest adopts that new CID when QEMU pushes the post-migration transport
	// reset. Binding to the stale CID leaves the listener unreachable and the host
	// times out against a guest that is running and healthy.
	l, err := vsock.ListenContextID(vsockCIDAny, uint32(cfg.RPCPort), &vsock.Config{})
	if err != nil {
