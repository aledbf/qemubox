...
_output/containerd-shim-beaconbox-v1: cmd/containerd-shim-beaconbox-v1 FORCE
	@echo "$(WHALE) $@"
	$(GO) build ${DEBUG_GO_GCFLAGS} ${GO_GCFLAGS} ${GO_BUILD_FLAGS} -o $@ ${GO_LDFLAGS} ${GO_TAGS} ./$<
ifeq ($(OS),Darwin)
	codesign --entitlements cmd/containerd-shim-beaconbox-v1/containerd-shim-beaconbox-v1.entitlements --force -s - $@
endif
...
verify-vendor: ## verify if all the go.mod/go.sum files are up-to-date
	...
	@cp -R ${ROOTDIR} ${TMPDIR}
	@(cd ${TMPDIR}/beaconbox && ${GO} mod tidy)
	@(cd ${TMPDIR}/beaconbox && ${GO} mod verify)
	diff -r -u ${ROOTDIR} ${TMPDIR}/beaconbox
	@rm -rf ${TMPDIR}
